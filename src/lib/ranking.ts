...
/**
 * period="month"(기본)면 now와 같은 (로컬) 달에 끝난 기록만, "all"이면 전체를 팀별로 합산해
 * 비용 내림차순(동률은 회의 수 내림차순 → 팀명 오름차순)으로 순위를 매긴다.
 */
export function rankTeams(
  ...
  for (const r of list) {
    ...
    if (period === "month" && (end.getFullYear() !== now.getFullYear() || end.getMonth() !== now.getMonth())) continue;
  ...
  return [...totals.entries()]
    .sort(
      (a, b) =>
        b[1].totalCost - a[1].totalCost || b[1].count - a[1].count || a[0].localeCompare(b[0]),
    )
    .map(([teamName, t], i) => ({
      rank: i + 1,
      teamName,
      ...
      count: t.count,
      sharePercent: sum > 0 ? Math.round((t.totalCost / sum) * 100) : 0,
    }));
}
