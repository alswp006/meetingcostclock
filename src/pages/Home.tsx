 ...
 *   MeetingCostClock -> the app's display name
 *   참석자 수와 평균 급여를 입력하면 회의가 진행되는 동안 실시간으로 누적 비용을 보여주는 직장인용 회의비용 계산기    -> the one-line description
 */
...
export default function Home() {
  ...
  return (
    <ScreenScaffold
      top={<Top title={<Top.TitleParagraph>MeetingCostClock</Top.TitleParagraph>} />}
    >
      {/* 시각 앵커: 헤드라인 + 카드 내 진입 버튼(부유 금지, display="block" 전체폭).
          데이터 앱이면 value를 <Amount typography="t1" />(핵심 숫자)로 교체하라. */}
      <SummaryHero
        label="MeetingCostClock"
        value={<Paragraph.Text typography="t2">참석자 수와 평균 급여를 입력하면 회의가 진행되는 동안 실시간으로 누적 비용을 보여주는 직장인용 회의비용 계산기</Paragraph.Text>}
